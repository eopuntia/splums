...
from models import Account, Role, Status, Note, Account_Equipment, Equipment, Event, Event_Type, Base
...
with Session() as session:
    ...
    renee = Account(account_id=212222, role=user, status=active, display_name="rez", 
                     given_name="Renee", surname="Rickert", photo_url="sample/renee/url")

    kahrl = Account(account_id=1234, role=administrator, status=active, display_name="zathras", 
                     given_name="Allin", surname="Kahrl", photo_url="sample/kahrl/url")

    estlin = Account(account_id=4321, role=attendant, status=active, display_name="estlin", 
                     given_name="Allin", surname="Mendez", photo_url="sample/estlin/url")
    ...
    renee_welding = Account_Equipment(account=renee, equipment=Equipment(name="Welding Station", icon_url="sample/url"), training_completed=False)
...
with Session() as session:
    ...
    unauthorized = session.scalar(select(Event_Type).where(Event_Type.name=="Unauthorized Entry"))
